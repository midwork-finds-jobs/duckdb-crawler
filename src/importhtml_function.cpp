...
enum class InferredType : uint8_t { VARCHAR = 0, BIGINT = 1, DOUBLE = 2 };
...
static bool TryParseBigInt(const string &str, int64_t &result) {
	if (str.empty())
		return true; // Empty is compatible with any type
...
static unique_ptr<GlobalTableFunctionState> ReadHtmlInitGlobal(ClientContext &context, TableFunctionInitInput &input) {
...
void RegisterReadHtmlFunction(ExtensionLoader &loader) {
	...
	TableFunction read_html_func("read_html", {LogicalType::VARCHAR, LogicalType::VARCHAR}, ReadHtmlFunction,
	                             ReadHtmlBind, ReadHtmlInitGlobal);
	...
	TableFunction read_html_func_idx("read_html", {LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::INTEGER},
	                                 ReadHtmlFunction, ReadHtmlBind, ReadHtmlInitGlobal);
