...
static unique_ptr<SingleCrawlResult> GetCachedEntry(Connection &conn, const string &url, int ttl_hours) {
	...
	auto result = conn.Query("SELECT url, status_code, content_type, body, error, response_time_ms "
	                         "FROM " +
	                             string(CACHE_TABLE_NAME) +
	                             " "
	                             "WHERE url = $1 AND cached_at > current_timestamp - INTERVAL '" +
...
static void SaveToCache(Connection &conn, const SingleCrawlResult &result) {
	...
	conn.Query(sql, result.url, result.status_code, result.content_type.empty() ? Value() : Value(result.content_type),
	           result.body.empty() ? Value() : Value(result.body), result.error.empty() ? Value() : Value(result.error),
...
static SingleCrawlResult CrawlSingleUrl(const string &url, const string &extraction_json, const string &user_agent,
                                        int timeout_ms) {
...
static unique_ptr<FunctionData> CrawlUrlBind(ClientContext &context, TableFunctionBindInput &input,
	...
	html_struct.push_back(make_pair(
	    "schema", LogicalType::MAP(LogicalType::VARCHAR, LogicalType::JSON()))); // MAP for schema['Product'] access
...
static unique_ptr<GlobalTableFunctionState> CrawlUrlInitGlobal(ClientContext &context, TableFunctionInitInput &input) {
	return make_uniq<CrawlUrlGlobalState>();
}

static unique_ptr<LocalTableFunctionState> CrawlUrlInitLocal(ExecutionContext &context, TableFunctionInitInput &input,
...
static OperatorResultType CrawlUrlInOut(ExecutionContext &context, TableFunctionInput &data, DataChunk &input,
                                        DataChunk &output) {
...
void RegisterCrawlUrlFunction(ExtensionLoader &loader) {
	...
	TableFunction func("crawl_url", {LogicalType::VARCHAR}, nullptr, CrawlUrlBind, CrawlUrlInitGlobal,
	                   CrawlUrlInitLocal);
	...
	TableFunction func_with_limit("crawl_url", {LogicalType::VARCHAR, LogicalType::BIGINT}, nullptr, CrawlUrlBind,
